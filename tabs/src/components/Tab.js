...
class Tab extends React.Component {
  constructor(props) {
    ...
    this.getParticipantsOnCall = this.getParticipantsOnCall.bind(this);
  }
  ...
  exchangeClientTokenForServerToken = async (token) => {
    ...
    let serverURL = `${process.env.REACT_APP_TEAMSFX_BACKEND_ENDPOINT}/getGraphAccessToken?ssoToken=${token}`;
   ...
   * or microsoftTeams.authentication.authenticate.
   */
     getParticipantsOnCall = async () => {
      let id = this.state.context['meetingId'];
      console.log("This is the meeting Id", id)
      let graphPhotoEndpoint = `https://graph.microsoft.com/v1.0/communications/calls/${id}/participants`;
      let graphRequestParams = {
        method: 'GET',
        headers: {
          'Content-Type': 'image/jpg',
  ...
  getParticipantInfo = async () => {
    ...
    let serverURL = `${process.env.REACT_APP_TEAMSFX_BACKEND_ENDPOINT}/getParticipantInfo?ssoToken=${ssoToken}&conversationId=${chatId}&aadObjectId=${userObjectId}&meetingId=${meetingId}`;
  ...
  displayInMeetingDialog() {
    ...
    let serverURL = `${process.env.REACT_APP_TEAMSFX_BACKEND_ENDPOINT}/inMeetingDialog?conversationId=${this.state.context.chatId}`;
